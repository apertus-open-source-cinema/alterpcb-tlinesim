...
const QString MainWindow::WINDOW_CAPTION =
...
static real_t FloatFromVData(const VData &data) {
  ...
  throw std::runtime_error(MakeString(
      "Expected float, got ", EnumToString(data.GetType()), " instead."));
}
...
static void MakeSweep(std::vector<real_t> &results, real_t min, real_t max,
                      real_t step) {
  size_t num = (size_t)std::max<ptrdiff_t>(
      rintp(std::max(0.0, max - min) / step + 0.5 + 1e-12), 1);
  results.clear();
  results.resize(num);
...
MainWindow::MainWindow() {
  ...
  QGroupBox *groupbox_type = new QGroupBox("Transmission line type", this);
  {
    ...
    connect(m_combobox_tline_types, SIGNAL(currentIndexChanged(int)), this,
            SLOT(OnUpdateTLineType()));
  ...
  QGroupBox *groupbox_parameters = new QGroupBox("Parameters", this);
  {
    ...
    m_scrollarea_parameters->setHorizontalScrollBarPolicy(
  ...
  {
    QLabel *label_simulation_type =
        new QLabel("Simulation type:", groupbox_simulation);
    ...
    m_pushbutton_simulate =
        new QPushButton(g_icon_simulation, "Simulate", this);
    ...
    m_lineedit_frequency_sweep_min =
        new QLineEditSmall("0.0", groupbox_simulation, 40);
    m_label_frequency_sweep[1] = new QLabel("to", groupbox_simulation);
    m_lineedit_frequency_sweep_max =
        new QLineEditSmall("10.0", groupbox_simulation, 40);
    m_label_frequency_sweep[2] = new QLabel("in steps of", groupbox_simulation);
    m_lineedit_frequency_sweep_step =
        new QLineEditSmall("1.0", groupbox_simulation, 40);
    m_label_frequency_sweep[3] = new QLabel("GHz", groupbox_simulation);
    m_label_frequency_sweep_file =
        new QLabel("Output File:", groupbox_simulation);
    m_lineedit_frequency_sweep_file = new QLineEdit(
        QDir::homePath() + "/frequency_sweep.txt", groupbox_simulation);
    m_pushbutton_frequency_sweep_browse =
        new QPushButton("Browse...", groupbox_simulation);
    ...
    m_lineedit_parameter_sweep_min =
        new QLineEditSmall("1.0", groupbox_simulation, 40);
    m_label_parameter_sweep[2] = new QLabel("to", groupbox_simulation);
    m_lineedit_parameter_sweep_max =
        new QLineEditSmall("2.0", groupbox_simulation, 40);
    m_label_parameter_sweep[3] = new QLabel("in steps of", groupbox_simulation);
    m_lineedit_parameter_sweep_step =
        new QLineEditSmall("0.1", groupbox_simulation, 40);
    m_label_parameter_sweep_file =
        new QLabel("Output File:", groupbox_simulation);
    m_lineedit_parameter_sweep_file = new QLineEdit(
        QDir::homePath() + "/parameter_sweep.txt", groupbox_simulation);
    m_pushbutton_parameter_sweep_browse =
        new QPushButton("Browse...", groupbox_simulation);
    ...
    m_label_parameter_tune[1] =
        new QLabel("Target Result:", groupbox_simulation);
    m_combobox_parameter_tune_target_result =
        new QComboBox(groupbox_simulation);
    m_label_parameter_tune[2] =
        new QLabel("Target Value:", groupbox_simulation);
    m_lineedit_parameter_tune_target_value =
        new QLineEdit("50.0", groupbox_simulation);
    ...
    connect(m_combobox_simulation_type, SIGNAL(currentIndexChanged(int)), this,
            SLOT(OnUpdateSimulationType()));
    connect(m_pushbutton_simulate, SIGNAL(clicked(bool)), this,
            SLOT(OnSimulate()));
    connect(m_combobox_parameter_sweep_parameter,
            SIGNAL(currentIndexChanged(int)), this,
            SLOT(OnUpdateSimulationType()));
    connect(m_combobox_parameter_tune_parameter,
            SIGNAL(currentIndexChanged(int)), this,
            SLOT(OnUpdateSimulationType()));
    connect(m_pushbutton_frequency_sweep_browse, SIGNAL(clicked(bool)), this,
            SLOT(OnFrequencySweepBrowse()));
    connect(m_pushbutton_parameter_sweep_browse, SIGNAL(clicked(bool)), this,
            SLOT(OnParameterSweepBrowse()));
  ...
  QGroupBox *groupbox_viewer = new QGroupBox("Viewer", this);
  {
    ...
    connect(m_slider_zoom, SIGNAL(valueChanged(int)), this,
            SLOT(OnZoomChange()));
    connect(m_combobox_image_type, SIGNAL(activated(int)), this,
            SLOT(OnImageTypeChange()));
    connect(m_checkbox_mesh_overlay, SIGNAL(clicked(bool)), this,
            SLOT(OnMeshOverlayChange()));
    connect(m_combobox_modes, SIGNAL(activated(int)), this,
            SLOT(OnModeChange()));
...

  {
    ...
    QLabel *label_about = new QLabel(
        "<a href=\"about\">About AlterPCB-TLineSim</a>", statusBar());
...
void MainWindow::LoadMaterials() {
  if (g_application_data_dir.isEmpty()) {
    statusBar()->showMessage(
        "Error: Could not load materials, data directory is missing.");
    return;
  }
  try {
    m_material_database->LoadFile(g_application_data_dir.toStdString() +
                                  "/materials.json");
    m_material_database->Finish();
  } catch (const std::runtime_error &e) {
    QMessageBox::critical(this, WINDOW_CAPTION,
                          QString("Error: Could not load material database: ") +
                              e.what(),
                          QMessageBox::Ok);
    return;
  }
...
void MainWindow::SimulationInit(TLineContext &context) {
  ...
  context.m_mesh_detail = exp2((real_t)(m_combobox_mesh_detail->currentIndex() -
                                        (int)MESHDETAIL_MEDIUM) *
                               0.5);
  ...
  for (size_t i = 0; i < tline_type.m_parameters.size(); ++i) {
    ...
    case TLINE_PARAMETERTYPE_REAL: {
      CustomLineEdit *lineedit_value =
          static_cast<CustomLineEdit *>(m_widget_parameters[i]);
    ...
    case TLINE_PARAMETERTYPE_MATERIAL_DIELECTRIC: {
      QComboBox *combobox_value =
          static_cast<QComboBox *>(m_widget_parameters[i]);
...
void MainWindow::SimulationShowResult(TLineContext &context) {
  ...
  assert(context.m_results.size() == TLINERESULT_COUNT *
                                         tline_type.m_modes.size() *
                                         context.m_frequencies.size());
  real_t *result_values =
      context.m_results.data() + TLINERESULT_COUNT * tline_type.m_modes.size() *
                                     (context.m_frequencies.size() - 1);
  for (size_t i = 0; i < tline_type.m_modes.size(); ++i) {
    for (size_t j = 0; j < TLINERESULT_COUNT; ++j) {
      m_lineedit_results[TLINERESULT_COUNT * i + j]->setText(
          QString::number(result_values[TLINERESULT_COUNT * i + j]));
    }
  }
...
void MainWindow::SimulateSingleFrequency() {
  ...
  // set frequency
  context.m_frequencies = {
      FloatFromString(m_lineedit_frequency->text().toStdString()) * 1e9};
...
void MainWindow::SimulateFrequencySweep() {
  ...
  // generate sweep
  real_t freq_min =
      FloatFromString(m_lineedit_frequency_sweep_min->text().toStdString()) *
      1e9;
  real_t freq_max =
      FloatFromString(m_lineedit_frequency_sweep_max->text().toStdString()) *
      1e9;
  real_t freq_step =
      FloatFromString(m_lineedit_frequency_sweep_step->text().toStdString()) *
      1e9;
  MakeSweep(context.m_frequencies, freq_min, freq_max, freq_step);

  // simulate
  QProgressDialogThreaded dialog("Frequency sweep ...", "Cancel", 0,
                                 (int)context.m_frequencies.size(), this);
  dialog.setWindowTitle(MainWindow::WINDOW_CAPTION);
  dialog.setMinimumDuration(0);
  dialog.execThreaded(
      [&](std::atomic<int> &task_progress, std::atomic<bool> &task_canceled) {
  ...
  // open output file
  std::string filename =
      m_lineedit_frequency_sweep_file->text().toLocal8Bit().constData();
  std::ofstream f;
  ...
  for (size_t i = 0; i < context.m_frequencies.size(); ++i) {
    ...
    real_t *results = context.m_results.data() +
                      TLINERESULT_COUNT * tline_type.m_modes.size() * i;
...
void MainWindow::SimulateParameterSweep() {
  ...
  // generate sweep
  real_t value_min =
      FloatFromString(m_lineedit_parameter_sweep_min->text().toStdString());
  real_t value_max =
      FloatFromString(m_lineedit_parameter_sweep_max->text().toStdString());
  real_t value_step =
      FloatFromString(m_lineedit_parameter_sweep_step->text().toStdString());
  std::vector<real_t> sweep_values;
  MakeSweep(sweep_values, value_min, value_max, value_step);

  // prepare input and output
  size_t param_index =
      (size_t)m_combobox_parameter_sweep_parameter
          ->itemData(m_combobox_parameter_sweep_parameter->currentIndex())
          .toInt();
  std::vector<real_t> combined_results;
  combined_results.resize(TLINERESULT_COUNT * tline_type.m_modes.size() *
                          sweep_values.size());

  // simulate
  QProgressDialogThreaded dialog("Parameter sweep ...", "Cancel", 0,
                                 (int)sweep_values.size(), this);
  dialog.setWindowTitle(MainWindow::WINDOW_CAPTION);
  dialog.setMinimumDuration(0);
  dialog.execThreaded([&](std::atomic<int> &task_progress,
                          std::atomic<bool> &task_canceled) {
    for (size_t i = 0; i < sweep_values.size(); ++i) {
      context.m_parameters[param_index].Value() = FloatScale(sweep_values[i]);
      tline_type.m_simulate(context);
      std::copy_n(context.m_results.data(), context.m_results.size(),
  ...
  // open output file
  std::string filename =
      m_lineedit_parameter_sweep_file->text().toLocal8Bit().constData();
  std::ofstream f;
  f.open(filename, std::ios_base::out | std::ios_base::trunc);
  if (f.fail())
    throw std::runtime_error("Could not open file '" + filename +
  ...
  for (size_t i = 0; i < sweep_values.size(); ++i) {
    f << sweep_values[i];
    real_t *results = combined_results.data() +
                      TLINERESULT_COUNT * tline_type.m_modes.size() * i;
    for (size_t j = 0; j < TLINERESULT_COUNT * tline_type.m_modes.size(); ++j) {
      f << '\t' << results[j];
    }
...
void MainWindow::SimulateParameterTune() {
  ...
  // get parameter
  size_t param_index =
      (size_t)m_combobox_parameter_tune_parameter
          ->itemData(m_combobox_parameter_tune_parameter->currentIndex())
          .toInt();

  // get target result
  size_t result_index =
      clamp<size_t>(m_combobox_parameter_tune_target_result->currentIndex(), 0,
                    TLINERESULT_COUNT * tline_type.m_modes.size() - 1);
  real_t target_value = FloatFromString(
      m_lineedit_parameter_tune_target_value->text().toStdString());

  // simulate
  real_t initial_value =
      FloatFromVData(context.m_parameters[param_index].Value());
  if (!FinitePositive(initial_value)) {
    initial_value =
        FloatFromVData(tline_type.m_parameters[param_index].m_default_value);
  }
  real_t root_value = FindRootRelative(
      [&context, &tline_type, param_index, result_index,
       target_value](real_t x) {
  ...
  CustomLineEdit *lineedit_value =
      static_cast<CustomLineEdit *>(m_widget_parameters[param_index]);
...
void MainWindow::OnUpdateTLineType() {
  ...
  m_tline_type = clamp<size_t>(m_combobox_tline_types->currentIndex(), 0,
                               g_tline_types.size() - 1);
  const TLineType &tline_type = g_tline_types[m_tline_type];
  const std::vector<MaterialConductor> &conductors =
      m_material_database->GetConductors();
  const std::vector<MaterialDielectric> &dielectrics =
      m_material_database->GetDielectrics();

  statusBar()->clearMessage();

  // update description
  m_textedit_description->setPlainText(
      QString::fromStdString(tline_type.m_description));
  ...
  {
    QWidget *widget = new QWidget(m_scrollarea_parameters),
            *widget_focus = m_scrollarea_parameters;
    QGridLayout *layout = new QGridLayout(widget);
    size_t row = 0;
    for (size_t i = 0; i < tline_type.m_parameters.size(); ++i) {
      const TLineParameter &parameter = tline_type.m_parameters[i];
      QLabel *label_name =
          new QLabel(QString::fromStdString(parameter.m_name) + ":", widget);
      ...
      case TLINE_PARAMETERTYPE_REAL: {
        CustomLineEdit *lineedit_value =
            new CustomLineEdit(parameter.m_unit_mm, widget);
        lineedit_value->SetText(
            QString::fromStdString(Json::ToString(parameter.m_default_value)));
      ...
      case TLINE_PARAMETERTYPE_MATERIAL_DIELECTRIC: {
        ...
        for (size_t j = 0; j < dielectrics.size(); ++j) {
          combobox_value->addItem(
              QString::fromStdString(dielectrics[j].m_name));
      ...
      if (parameter.m_type == TLINE_PARAMETERTYPE_REAL) {
        m_combobox_parameter_sweep_parameter->addItem(
            QString::fromStdString(parameter.m_name), (int)i);
        m_combobox_parameter_tune_parameter->addItem(
            QString::fromStdString(parameter.m_name), (int)i);
      }
    }
  ...
  {
    QWidget *widget = new QWidget(m_scrollarea_results),
            *widget_focus = m_scrollarea_results;
    QGridLayout *layout = new QGridLayout(widget);
    size_t row = 0;
    for (size_t i = 0; i < tline_type.m_modes.size(); ++i) {
      for (size_t j = 0; j < TLINERESULT_COUNT; ++j) {
        QLabel *label_name =
            new QLabel(QString::fromStdString(tline_type.m_modes[i]) + " " +
                           TLINERESULT_NAMES[j] + ":",
                       widget);
        ...
        layout->addWidget(label_unit, (int)row, 2);
        ++row;
        m_combobox_parameter_tune_target_result->addItem(
            QString::fromStdString(tline_type.m_modes[i]) + " " +
            TLINERESULT_NAMES[j]);
      }
...
void MainWindow::OnUpdateSimulationType() {
  ...
  MultiGroupVisible({
      ...
      {{m_label_frequency_sweep[0], m_label_frequency_sweep[1],
        m_label_frequency_sweep[2], m_label_frequency_sweep[3],
        m_lineedit_frequency_sweep_min, m_lineedit_frequency_sweep_max,
        m_lineedit_frequency_sweep_step, m_label_frequency_sweep_file,
        m_lineedit_frequency_sweep_file, m_pushbutton_frequency_sweep_browse},
       simulation_type == SIMULATION_FREQUENCY_SWEEP},
      {{m_label_parameter_sweep[0], m_label_parameter_sweep[1],
        m_label_parameter_sweep[2], m_label_parameter_sweep[3],
        m_combobox_parameter_sweep_parameter, m_lineedit_parameter_sweep_min,
        m_lineedit_parameter_sweep_max, m_lineedit_parameter_sweep_step,
        m_label_parameter_sweep_file, m_lineedit_parameter_sweep_file,
        m_pushbutton_parameter_sweep_browse},
       simulation_type == SIMULATION_PARAMETER_SWEEP},
      {{m_label_parameter_tune[0], m_label_parameter_tune[1],
        m_label_parameter_tune[2], m_combobox_parameter_tune_parameter,
        m_combobox_parameter_tune_target_result,
        m_lineedit_parameter_tune_target_value},
...
void MainWindow::OnFrequencySweepBrowse() {
  QString selected_file = QFileDialog::getSaveFileName(
      this, "Save results as", m_lineedit_frequency_sweep_file->text(),
...
void MainWindow::OnParameterSweepBrowse() {
  QString selected_file = QFileDialog::getSaveFileName(
      this, "Save results as", m_lineedit_parameter_sweep_file->text(),
      "Text Files (*.txt);;All Files (*)");
  if (selected_file.isEmpty())
    return;
  QFileInfo fi(selected_file);
...
void MainWindow::OnImageTypeChange() {
  m_meshviewer->SetImageType(
      (MeshImageType)m_combobox_image_type->currentIndex());
}
...
void MainWindow::OnModeChange() {
  m_meshviewer->SetMode(
      clamp<size_t>(m_combobox_modes->currentIndex(), 0,
                    g_tline_types[m_tline_type].m_modes.size() - 1));
}
